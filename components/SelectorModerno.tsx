  ...
  return (
    <div className="relative" ref={contenedorRef}>
      <button
        ...
        `}
      >
        <span className={opcionSeleccionada ? "text-neutral-900 dark:text-white font-medium" : "text-neutral-400 dark:text-neutral-500"}>
          {opcionSeleccionada ? opcionSeleccionada.label : placeholder}
        </span>
        <svg
          viewBox="0 0 24 24"
          className={`w-4 h-4 text-neutral-400 dark:text-neutral-500 shrink-0 transition-transform ${abierto ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
        >
      ...
      <div
        className={`absolute z-50 mt-2 w-full bg-white dark:bg-neutral-900 ring-1 ring-black/5 dark:ring-white/10 rounded-2xl shadow-2xl overflow-hidden
          ...
          {opciones.map((o) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm transition
                ${o.value === value ? "bg-orange-50 dark:bg-orange-500/10 text-orange-700 dark:text-orange-400 font-medium" : "text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700"}
              `}
            >
              {o.label}
            </button>
